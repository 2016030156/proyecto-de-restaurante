...
public class ListaRecepcionistasActivity extends AppCompatActivity implements Response.Listener<JSONObject>, Response.ErrorListener {


    ListView lstRecepcionistas;
    Button btnNuevo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lista_recepcionistas);
        lstRecepcionistas = (ListView) findViewById(R.id.lstRecepcionistas);
        btnNuevo = (Button) findViewById(R.id.btnNuevoRecepcionista);
        btnNuevo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ListaRecepcionistasActivity.this,NuevoUsuarioActivity.class);
                Bundle bundle = new Bundle();
                bundle.putString("tipo","Agregar Recepcionista");
                bundle.putString("perfil","4");
                intent.putExtras(bundle);
                startActivityForResult(intent,0);
            }
        });
        request = Volley.newRequestQueue(this);
        consultarRecepcionistas();
    }

    public void consultarRecepcionistas() {
        String url = serverip + "wsJSONCargarRecepcionistas.php";
    ...
    public void onResponse(JSONObject response) {
        Usuarios recepcionista = null;
        ArrayList<Usuarios> lista = new ArrayList<>();
        JSONObject jsonObject;
        JSONArray json = response.optJSONArray("recepcionistas");
        try {
            for (int i = 0; i < json.length(); i++) {
                recepcionista = new Usuarios();
                jsonObject = json.getJSONObject(i);
                recepcionista.setId(jsonObject.optString("id"));
                recepcionista.setNombre(jsonObject.optString("nombre"));
                recepcionista.setPaterno(jsonObject.optString("paterno"));
                recepcionista.setMaterno(jsonObject.optString("materno"));
                recepcionista.setUsuario(jsonObject.optString("usuario"));
                recepcionista.setContrasena(jsonObject.optString("contra"));
                recepcionista.setTipo("Recepcionista");
                recepcionista.setEstado(jsonObject.optString("status"));
                recepcionista.setFecha(jsonObject.optString("create_at"));
                lista.add(recepcionista);
            }
            adapter = new MyArrayAdapter(this, R.layout.layout_usuario, lista);
            lstRecepcionistas.setAdapter(adapter);
